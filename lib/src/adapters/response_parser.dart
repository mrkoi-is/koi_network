/// Koi 响应解析器接口
/// Koi Response Parser Interface
///
/// 用于解耦网络库和项目特定的后端响应格式。
/// Used to decouple the network library from project-specific backend response formats.
///
/// 每个项目根据自己的后端响应格式实现此接口。
/// Each project implements this interface based on its own backend response format.
///
/// ## 示例 / Example
///
...
abstract class KoiResponseParser {
  ...
  /// 提取消息（错误消息或成功消息）
  /// Extract message (error message or success message)
  ...
  /// 判断是否为认证错误（如 token 过期、权限不足等）
  /// Determine if it is an authentication error (e.g. token expired, insufficient permissions)
  ///
...
/// 默认响应解析器（标准 {code, msg, data} 格式）
/// Default response parser (Standard {code, msg, data} format)
///
/// 兼容常见的后端响应格式：
/// Compatible with common backend response formats:
/// ```json
...
class KoiDefaultResponseParser implements KoiResponseParser {
  ...
  /// 认证错误 HTTP 状态码
  /// Authentication error HTTP status codes
